<?php
/**
 * Create Role & Configure Permissions Matrix
...
$definedModules = [
    ...
    'contact' => [
        'name' => 'ข้อมูลการติดต่อและบริษัท (Contact Info)',
        'actions' => [
    ...
    'users' => [
        ...
        'actions' => [
            ...
            'edit'   => ['code' => 'users.edit', 'name' => 'แก้ไขผู้ดูแลระบบ', 'col' => 'update'],
            'delete' => ['code' => 'users.delete', 'name' => 'ลบผู้ดูแลระบบ', 'col' => 'delete'],
        ]
    ],
    'roles' => [
        'name' => 'การจัดการบทบาทและสิทธิ์ (Roles & Matrix)',
        'actions' => [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation
    if ($name === '' || $slug === '') {
        $error = 'กรุณากรอกชื่อบทบาทและรหัสบทบาท (Slug) ให้ครบถ้วน';
    } elseif (!preg_match('/^[a-z0-9_]{2,50}$/', $slug)) {
            ...
            try {
                ...
                if (!empty($selectedPermIds)) {
                    ...
                    foreach ($selectedPermIds as $permId) {
                        if ($permId > 0) {
                            $rpInsert->execute(['role_id' => $newRoleId, 'permission_id' => $permId]);
                        }
                    }
                }

                db()->commit();
                flash('success', 'สร้างบทบาท "' . $name . '" พร้อมกำหนดสิทธิ์เรียบร้อยแล้ว');
...
<div class="mx-auto w-full max-w-none px-2 pb-8 pt-1 text-sm md:px-4 lg:px-8 space-y-6">
    ...
    <form method="POST" action="create.php" id="roleMatrixForm" style="display: flex; flex-direction: column; gap: 1.5rem;">
        <?= csrf_field() ?>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.25rem;">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="<?= e($name) ?>" required
                        placeholder="เช่น ผู้จัดการฝ่ายการตลาด, เจ้าหน้าที่ไอที"
                ...
                <div>
                    <label style="display: block; font-size: 0.75rem; font-weight: 600; color: #334155; margin-bottom: 0.5rem;" for="slug">
                        รหัสบทบาท (Slug ID) <span style="color: #ef4444;">*</span>
                    </label>
                    <input type="text" id="slug" name="slug" value="<?= e($slug) ?>" required
                        placeholder="เช่น marketing_manager, it_support"
                        style="width: 100%; height: 42px; padding: 0 1rem; border-radius: 0.75rem; border: 1px solid #cbd5e1; background: #ffffff; font-family: monospace; font-size: 0.8125rem; color: #0f172a; outline: none; transition: border-color 0.15s;"
                        onfocus="this.style.borderColor='#0f172a';"
                        onblur="this.style.borderColor='#cbd5e1';">
                </div>
                        ...
                        <tr>
                            ...
                            <th style="padding: 0.875rem 1.25rem; width: 40%; min-width: 260px;">
                                <label style="display: inline-flex; align-items: center; gap: 0.75rem; cursor: pointer; user-select: none;">
                                    ...
                                    <span style="font-weight: 700; font-size: 0.75rem; color: #0f172a;">โมดูล (Module)</span>
                                </label>
                            </th>
