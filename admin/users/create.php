...
$email = '';
$fullName = '';
$selectedRoleId = !empty($allRoles) ? (int)$allRoles[0]['id'] : 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $email === '' || $password === '' || $confirmPassword === '' || !$selectedRole) {
        $error = 'กรุณากรอกข้อมูลในช่องที่มีเครื่องหมาย * ให้ครบถ้วน และเลือกบทบาทที่ถูกต้อง';
        ...
        if ($existing) {
            ...
            try {
                $insertStmt = db()->prepare(
                    'INSERT INTO admins (username, email, password_hash, full_name, role, role_id, created_at, updated_at) 
                     VALUES (:username, :email, :password_hash, :full_name, :role, :role_id, NOW(), NOW())'
                );
                $insertStmt->execute([
                    'username'      => $username,
                    'email'         => $email,
...
<div class="mx-auto w-full max-w-none px-2 pb-8 pt-1 text-sm md:px-4 lg:px-8 space-y-6">
    ...
    <!-- Create Form -->
    <form id="createAdminForm" method="post" action="create.php" novalidate style="display: flex; flex-direction: column; gap: 1.5rem;">
        <?= csrf_field() ?>
            ...
            <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 1rem; margin-bottom: 1.25rem;">
                ...
                <div style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
                    ...
                    <?php if (has_permission('roles.create')): ?>
                        <a href="<?= ADMIN_URL ?>/roles/create.php" target="_blank"
        ...
        <!-- Section 3: Password -->
        <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 1rem; padding: 1.5rem 1.75rem; box-shadow: 0 1px 2px rgba(0,0,0,0.02);">
            <h2 style="font-size: 1rem; font-weight: 700; color: #0f172a; margin: 0 0 0.25rem 0;">กำหนดรหัสผ่าน (Password & Security)</h2>
            <p style="font-size: 0.75rem; color: #94a3b8; margin: 0 0 1.25rem 0;">รหัสผ่านต้องมีความยาวอย่างน้อย 6 ตัวอักษร</p>

            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.25rem;">
                <!-- Password -->
                <div>
                    <label style="display: block; font-size: 0.75rem; font-weight: 600; color: #334155; margin-bottom: 0.5rem;" for="reg_password">
                    ...
                    <div style="position: relative; display: flex; align-items: center;">
                        <input type="password" name="password" id="reg_password" required
                            ...
                            onfocus="this.style.borderColor='#0f172a';"
                            onblur="this.style.borderColor='#cbd5e1';">
                        <button type="button" onclick="togglePasswordVisibility('reg_password', this)"
                ...
                <!-- Confirm Password -->
                <div>
                    <label style="display: block; font-size: 0.75rem; font-weight: 600; color: #334155; margin-bottom: 0.5rem;" for="reg_confirm_password">
                        ยืนยันรหัสผ่าน (Confirm Password) <span style="color: #ef4444;">*</span>
                    </label>
                    <div style="position: relative; display: flex; align-items: center;">
                        <input type="password" name="confirm_password" id="reg_confirm_password" required
                            ...
                            onfocus="this.style.borderColor='#0f172a';"
                            onblur="this.style.borderColor='#cbd5e1';">
                        <button type="button" onclick="togglePasswordVisibility('reg_confirm_password', this)"
                            style="position: absolute; right: 0.75rem; background: none; border: none; padding: 0.25rem; color: #94a3b8; cursor: pointer; display: flex; align-items: center;">
                            <svg style="width: 16px; height: 16px;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            ...
            <div id="pwd_checklist" style="display: none; margin-top: 1.25rem; padding: 1rem; border-radius: 0.75rem; background-color: #f8fafc; border: 1px solid #e2e8f0;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 0.5rem; font-size: 0.75rem;">
                    <div id="rule_len" style="display: flex; align-items: center; gap: 0.5rem; color: #94a3b8;">
                        <span id="icon_rule_len">✕</span>
                        <span>ความยาวอย่างน้อย 6 ตัวอักษร</span>
                    </div>
                    <div id="rule_lower" style="display: flex; align-items: center; gap: 0.5rem; color: #94a3b8;">
                        <span id="icon_rule_lower">✕</span>
                        <span>มีตัวพิมพ์เล็ก (a-z) อย่างน้อย 1 ตัว</span>
                    </div>
                    <div id="rule_num" style="display: flex; align-items: center; gap: 0.5rem; color: #94a3b8;">
                        <span id="icon_rule_num">✕</span>
                        <span>มีตัวเลข (0-9) อย่างน้อย 1 ตัว</span>
                    </div>
                    <div id="rule_match" style="display: flex; align-items: center; gap: 0.5rem; color: #94a3b8;">
...
function validatePasswordLive() {
    ...
    updateCheckItem('rule_len', 'icon_rule_len', hasLen);
