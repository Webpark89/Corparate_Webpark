...
// คำนวณจำนวนรายการทั้งหมดสำหรับการแบ่งหน้า (Pagination)
$totalSt = db()->prepare("SELECT COUNT(*) FROM portfolio p $whereSql");
        ...
        <tbody class="divide-y divide-slate-100 bg-white">
          <?php foreach ($portfolios as $row): ?>
            <tr class="hover:bg-slate-50/50 transition-colors cursor-pointer js-clickable-row"
              ...
              <td class="px-4 py-3">
                <img src="<?= e(upload_url($row['cover_image']) ?: 'https://picsum.photos/seed/p' . $row['id'] . '/120/80') ?>"
              ...
              <td class="px-4 py-3 text-right" onclick="event.stopPropagation();">
                <div class="inline-flex overflow-hidden rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <button type="button"
                    onclick="if(confirm('ยืนยันการลบผลงานนี้?')) window.location.href='delete.php?id=<?= $row['id'] ?>'"
    ...
    <nav class="mt-4 flex justify-center" aria-label="Pagination">
      <ul class="inline-flex items-center gap-1">
        <?php for ($i = 1; $i <= $pg['pages']; $i++):
          ...
          <li>
            <a href="?<?= e($qs) ?>"
              class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-xs font-semibold transition-colors <?= $i === $pg['current'] ? 'bg-slate-900 text-white shadow-sm' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-50' ?>">
              <?= $i ?>
            </a>
          </li>
        <?php endfor; ?>
